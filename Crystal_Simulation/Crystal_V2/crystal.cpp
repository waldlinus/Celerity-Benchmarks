...
int main(int argc, char *argv[]) {
...

    {
        ...
        celerity::buffer<factor, 2> output_buf((cl::sycl::range<2>(nr, 2)));
        ...
        queue.submit([=](celerity::handler &cgh) {
            ...
            auto res_E = output_buf.get_access<cl::sycl::access::mode::discard_write>(cgh, celerity::access::slice<2>(1));
            cgh.parallel_for<class CrystalKernel>(cl::sycl::range<1>(nr), [=](cl::sycl::item<1> item) {
                factor A = 0.0f;
                factor B = 0.0f;
                factor arg;
                for (size_t i = 0; i < na; i++) {
                    factor A1, B1;
                    arg = twopi * (buf_h[DIM2_H * item[0] + 0] * buf_a[DIM2_A * i + 1] +
                                   buf_h[DIM2_H * item[0] + 1] * buf_a[DIM2_A * i + 2] +
                                   buf_h[DIM2_H * item[0] + 2] * buf_a[DIM2_A * i + 3]);
                    A1 = cl::sycl::cos(arg);
                    A += buf_a[DIM2_A * i] * A1;
                ...
                if (sequential_check) {
                    ...
                    for (size_t i = 0; i < nr; i++) {
                        for (size_t j = 0; j < 2; j++) {
                            factor diff = result[{i, j}] - sequential[i * 2 + j];
                            if (diff > 10E-3 || diff < -10E-3) {
                                std::cout << "Error at position " << i * 2 + j << ", expected value " << sequential[i * 2 + j] << ", actual value " << result[{i, j}] << std::endl;
